...
class AsyncDotLoader:
    """
    Async version of dot loader animation.
    ...
    def __init__(self, prompt: str, adaptive_buffer: Any = None, 
        ...
        self.content_ready = asyncio.Event()
    ...
    async def _replay_chunks(self, stored: List[Tuple[str, float]]) -> Tuple[str, str]:
        """Replay stored chunks with original timing preserved."""
        if not stored:
            return "", ""
    ...
    async def run_with_loading(self, stream: Any) -> Tuple[str, str]:
        """Process stream with loading animation, maintaining exact behavior."""
        if not self.buffer:
                    ...
                    try:
                        ...
                        if txt:
                            if first_chunk:
                                self.resolved = True
                                if not self.no_anim:
                                    # Critical: Wait for animation to complete before
                                    # starting content output
                                    await self.content_ready.wait()
                                first_chunk = False
                            
                            now = time.time()
                            if store_mode:
                                if not self.content_ready.is_set():
