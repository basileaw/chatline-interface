...
@dataclass
class Pattern:
    name: str; start: str; end: str; color: Optional[str]; italic: bool; remove_delimiters: bool
...
class OutputHandler:
    def __init__(self, patterns: List[Dict] = None, base_color: str = COLORS['GREEN']):
        ...
        for p in self.patterns:
            if p.start in used_chars or p.end in used_chars: raise ValueError(f"Pattern '{p.name}' uses a character that's already in use")
    ...
    def get_style(self) -> str:
        color, italic = self.base_color, False
    ...
    def process_chunk(self, chunk: str) -> str:
        if not chunk: return ""
        output, i = [], 0
        output.append(FORMATS['ITALIC_OFF'] + self.base_color if not self.active_patterns else '')
        while i < len(chunk):
            ch = chunk[i]
            if i == 0 or chunk[i-1].isspace(): output.append(self.get_style())
            if self.active_patterns and ch in self.end_map and ch == self.by_name[self.active_patterns[-1]].end:
                ...
                if not current_pat.remove_delimiters: output.append(self.get_style() + ch)
            ...
            if ch in self.start_map:
                ...
                output.append(self.get_style())
                if not new_pat.remove_delimiters: output.append(ch)
                i += 1
                continue
            output.append(ch)
            i += 1
    ...
    def process_and_write(self, chunk: str) -> tuple[str, str]:
        ...
        sys.stdout.write(styled_text); sys.stdout.flush()
        return chunk, styled_text

class RawOutputHandler:
    def __init__(self): pass
    def process_and_write(self, chunk: str) -> tuple[str, str]:
        sys.stdout.write(chunk); sys.stdout.flush(); return chunk, chunk