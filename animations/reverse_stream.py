...
class ReverseStreamer:
    ...
    def __init__(self, utilities: Utilities, painter: Painter, delay: float = 0.08):
        if not painter:
    ...
    def get_style(self, active_patterns: List[str]) -> str:
        ...
        style = color
        if italic: style += self.painter.get_format('ITALIC_ON')
        if bold: style += self.painter.get_format('BOLD_ON')
        return style
    ...
    def format_lines(self, lines: List[List[StyledWord]]) -> str:
        ...
        formatted_lines = []
        current_style = self.painter.get_format('RESET') + self.painter.base_color
        ...
        if current_style != self.painter.get_format('RESET') + self.painter.base_color:
            result += self.painter.get_format('RESET') + self.painter.base_color
