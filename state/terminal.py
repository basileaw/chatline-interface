...
class TerminalManager:
    def __init__(self, utilities):
        self.utils = utilities  # Keep for now as we transition
        self.prompt_session = PromptSession()
        self._term_width = self._get_terminal_width()  # Use internal method

    def _get_terminal_width(self) -> int:
        """Get current terminal width."""
        return shutil.get_terminal_size().columns

    def _write(self, text: str = "", style: str = None, newline: bool = False):
        """Internal method to write to stdout and flush."""
        if style: sys.stdout.write(self.utils.get_format(style))
        sys.stdout.write(text)
        if style: sys.stdout.write(self.utils.get_format('RESET'))
    ...
    async def handle_scroll(self, styled_lines: str, prompt: str, delay: float = 0.5):
        ...
        for i in range(len(lines) + 1):
            ...
            self._write(self.utils.get_format('RESET') + prompt)
            time.sleep(delay)
